...
program
    ...
    .description(`creates a new functional component in the 'components/component-name' directory with the following file structure:
...
the component can then be imported with:
    ...
    .option('-m, --module', `creates and imports 'component-name.module.css' file into component-name.js`, false)
    .option('-d, --dir <pathToParentDir>', 'specify the path to the parent directory', 'src/components')
...
// check 'components' directory if default directory was not found
    ...
    if (exists(fullParentDir)) {
        ...
        error(`Could not find a 'src/components' or 'components' directory!\nPlease create one and try again, or specify another diretory with '-d <directory-name>'.`);
        process.exit(0);
    }
}
else {
    error(`The directory '${fullParentDir}' does not exist! Please create it and try again.`);
    process.exit(0);
}
// The directory of the component (Default: components/${componentName})
const componentDir = normalize(`${options.dir}/${componentName}`);
// Component file path
const componentPath = normalize(`${componentDir}/${componentName}${options.typescript ? '.tsx' : '.js'}`);
...
`;
// Index file path
const indexPath = normalize(`${componentDir}/index${options.typescript ? '.tsx' : '.js'}`);
// Index file template
...
if (exists(componentDir)) {
    error(`'${componentDir}' already exists! Please delete the directory and try again.`);
    process.exit(0);
}
else {
...
fs.mkdir(componentDir)
    ...
    if (options.module) {
        const modulePath = normalize(`${componentDir}/${componentName}.module.css`);
